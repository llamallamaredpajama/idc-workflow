...
we refuse to run an unverifiable workflow command. RECOVERY commands `doctor|update|uninstall` (and
`init`, which repairs/creates the scaffold) may still expand on an unknown/invalid legacy receipt so
the operator can diagnose or migrate the repo — but they are STILL blocked on a positively stale
runtime, because stale recovery code is itself unsafe. `janitor` (a board-mutating maintenance sweep)
is treated as fail-closed like a workflow command.
...
WORKFLOW_COMMANDS = {"think", "intake", "plan", "build", "recirculate", "autorun", "janitor"}
# May still expand on an unknown/invalid legacy receipt so the operator can diagnose or migrate.
ALLOW_ON_INVALID = {"doctor", "update", "uninstall", "init"}
...
def _fail_closed_or_allow(command, why):
    """The unverifiable-freshness fork: fail CLOSED for workflow/janitor commands (block with the
    repair message); ALLOW recovery/init commands (they must run to diagnose or migrate the repo)."""
    if command in ALLOW_ON_INVALID:
        H.warn(f"idc-entry-gate: allowing recovery command /idc:{command} despite: {why}")
        H.prompt_expansion_context(_context(command))
...
def _admit(payload, plugin_root, command):
    ...
    if result.running_version is None:
        ...
        _fail_closed_or_allow(command, "the running plugin manifest could not be read")
...
def main():
    ...
    try:
        ...
        _fail_closed_or_allow(command, f"unexpected gate error: {exc}")
    sys.exit(0)
