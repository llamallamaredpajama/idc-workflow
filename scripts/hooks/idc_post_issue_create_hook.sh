#!/bin/bash
# idc_post_issue_create_hook.sh — the PostToolUse wrapper for the issue-create-add board-coherence
...
# per CLAUDE.md).
#
# Contract (mirrors the PreToolUse interlock wrapper + the sibling commit-sync wrapper):
...
#     Bash tool + a successful `gh issue create` not already board-added in the same command.
...
[ -n "$ROOT" ] && [ -f "$ROOT/scripts/hooks/idc_post_issue_create.py" ] || exit 0

# Cheap pre-filter: the matcher is every Bash call, but only a `gh issue create` is relevant. Skip
# spawning python for the ~all Bash calls that don't mention "issue" (a single token, immune to the
# whitespace-flexible `gh  issue  create` forms the observer itself still matches precisely). Behaviour-
