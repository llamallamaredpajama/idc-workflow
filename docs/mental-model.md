# The IDC mental model — the pipeline

> One picture for the whole system. IDC is a **pipeline**: an idea goes in one end, working
> software comes out the other, and the flow only ever stops to ask you one question — at the top.

<p align="center">
  <img src="assets/pipeline-hero.png" alt="The IDC pipeline as a labeled rig — an idea in, working software out" width="100%">
</p>

<sub>The painted diagram above is the IDC pipeline drawn as a labeled rig — every numbered fixture
maps to a part named below. It's flattened from the landing-page source in `docs/site/`.</sub>
...
| Part of the pipeline | What it actually is in IDC | Command / file |
|---|---|---|
...
| ╳ **Diverter #1** *(the one gate)* | The single human checkpoint, at the **end of Think**: a reviewable **Think PR** carrying the PRD + TRD. They stay **draft until you merge** — **merge = approval = admission**. Approve in-session or later from your phone. **Nothing else in the pipeline ever asks you for anything** — Autorun and Build never improvise a gate or re-ask a decision you've already made. | the Think PR (`idc:idc-gate-issue`) |
...
| ╳ **Diverter #2** *(ship or return)* | At the end of Build the finisher routes the flow: **clean** → out the Faucet into the Glass; **not-good** → into the **Recirculator**. **This valve has no lock** — it isn't a human gate; it **ships automatically the moment every test is green**. | the finisher's ship/return decision |
...
| 🚰 **Faucet** | Open it and the whole pipeline runs on its own — draining every idea in the tank to shipped software, hands-off. (Run the stages by hand and you're just working the faucet manually.) | `/idc:autorun` |
...
## Following one drop through the pipeline
...
2. **It hits Diverter #1 — the one gate.** Think opens a **Think PR** carrying the PRD + TRD and lands
   ...
   waiting on you. **This is the only place the whole pipeline ever asks you for anything.**
...
most detailed part of the pipeline, so here it is on its own.
...
The board is the pipeline's instrumentation: a sensor on every component, a status light on every
...
| `Stage` | `Consideration · Planning · Buildable` — **which part of the pipeline** the drop is in (`Consideration` = an open Think PR, pending admission at the gate) |
...
## The pipeline itself — install & upkeep

Four commands aren't about the *water* at all — they build and maintain the **pipeline** itself:

| Command | Plumbing job | What it does |
|---|---|---|
| `/idc:init` | **Install the pipeline** | Scaffolds the governance contract + config, provisions the board, sets the type-aware TRD gate (brownfield on / greenfield off) by scanning + confirming what's already there, enables IDC for this repo only, writes an install receipt. |
| `/idc:doctor` | **Pressure-test & inspect** | Read-only health check — confirms the pipeline is wired correctly (and fails loudly if IDC leaked on at global scope). |
| `/idc:update` | **Upgrade the fittings** | Refreshes the stamped scaffold files after a plugin version bump. |
| `/idc:uninstall` | **Remove the pipeline** | Rips out IDC's footprints in one revertable commit. |

## Why it's plumbed this way

The pipeline isn't decoration — each part is a guardrail that earns its place:
