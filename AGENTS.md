...
contents, the `ke-snap` commands, teardown — live in `docs/dev/local-e2e-testing.md`; read it before
an e2e run.** This section is the *how-to-drive-it* playbook for when the user asks to test a change.
...
- **Full GitHub-fidelity e2e — must execute in a separate session INSIDE a sandbox.** You cannot run
  ...
  repo's agent **sets it up and reads the results** — it does not run the e2e inline. (The `/idc:*`
  lifecycle is Claude-Code-only; Codex's IDC support is an adapter, not the interactive flow.)
