// ─────────────────────────────────────────────────────────────────────────────
// VENDORED from pi-harnesses · upstream path: extensions/coms-net.ts
...
// IDC-LOCAL — glass-wall directional ACL for the coms_net_send seam (issue #27, B3).
import { evaluateComsNetSendForRole } from "./idc-role-harness.ts";
...
// Fleet teardown opt-out. Default ON: only the literal "0" disables exiting.
export function comsNetExitOnServerShutdownEnabled(raw: string | undefined): boolean {
	return raw !== "0";
}
const EXIT_ON_SERVER_SHUTDOWN = comsNetExitOnServerShutdownEnabled(process.env.PI_COMS_NET_EXIT_ON_SERVER_SHUTDOWN);
...
export function prepareComsNetResponsePayload(
	lastAssistantText: string,
	response_schema?: object | null,
): { payload: any; error: string | null } {
	if (!response_schema || typeof response_schema !== "object") {
		return { payload: lastAssistantText, error: null };
	}
	let payload: any;
	try {
		payload = JSON.parse(lastAssistantText);
	} catch {
		return { payload: null, error: "response not valid JSON" };
	}
	const validation = validateJsonSchemaValue(response_schema, payload);
	if (!validation.valid) {
		return {
			payload: null,
			error: `response did not match schema: ${validation.reason ?? "schema mismatch"}`,
		};
	}
	return { payload, error: null };
}

function validateJsonSchemaValue(schema: unknown, value: any, pathLabel = "$", rootSchema: unknown = schema): { valid: boolean; reason?: string } {
	if (schema === true) return { valid: true };
	if (schema === false) return { valid: false, reason: `${pathLabel} is disallowed by schema` };
	if (!schema || typeof schema !== "object" || Array.isArray(schema)) {
		return { valid: false, reason: `${pathLabel} has an invalid schema` };
	}
	const s = schema as Record<string, any>;

	if (typeof s.$ref === "string") {
		const refSchema = resolveLocalSchemaRef(rootSchema, s.$ref);
		if (refSchema === undefined) return { valid: false, reason: `${pathLabel} has unresolved schema ref ${s.$ref}` };
		return validateJsonSchemaValue(refSchema, value, pathLabel, rootSchema);
	}

	if (Array.isArray(s.allOf)) {
		for (const child of s.allOf) {
			const result = validateJsonSchemaValue(child, value, pathLabel, rootSchema);
			if (!result.valid) return result;
		}
	}
	if (Array.isArray(s.anyOf) && !s.anyOf.some((child: unknown) => validateJsonSchemaValue(child, value, pathLabel, rootSchema).valid)) {
		return { valid: false, reason: `${pathLabel} did not match any allowed schema` };
	}
	if (Array.isArray(s.oneOf)) {
		const matches = s.oneOf.filter((child: unknown) => validateJsonSchemaValue(child, value, pathLabel, rootSchema).valid).length;
		if (matches !== 1) return { valid: false, reason: `${pathLabel} matched ${matches} schemas, expected exactly one` };
	}
	if (s.not !== undefined && validateJsonSchemaValue(s.not, value, pathLabel, rootSchema).valid) {
		return { valid: false, reason: `${pathLabel} matched a forbidden schema` };
	}

	if (s.const !== undefined && !deepEqualJson(value, s.const)) {
		return { valid: false, reason: `${pathLabel} expected const ${JSON.stringify(s.const)}` };
	}
	if (Array.isArray(s.enum) && !s.enum.some((candidate: unknown) => deepEqualJson(value, candidate))) {
		return { valid: false, reason: `${pathLabel} expected one of ${JSON.stringify(s.enum)}` };
	}

	if (s.type !== undefined) {
		const allowed = Array.isArray(s.type) ? s.type : [s.type];
		if (!allowed.some((typeName) => typeof typeName === "string" && matchesJsonSchemaType(typeName, value))) {
			return { valid: false, reason: `${pathLabel} expected ${allowed.join("|")}, got ${jsonTypeOf(value)}` };
		}
	}

	if (typeof value === "string") {
		if (typeof s.minLength === "number" && value.length < s.minLength) return { valid: false, reason: `${pathLabel} shorter than minLength ${s.minLength}` };
		if (typeof s.maxLength === "number" && value.length > s.maxLength) return { valid: false, reason: `${pathLabel} longer than maxLength ${s.maxLength}` };
		if (typeof s.pattern === "string") {
			try {
				if (!new RegExp(s.pattern).test(value)) return { valid: false, reason: `${pathLabel} did not match pattern ${s.pattern}` };
			} catch {
				return { valid: false, reason: `${pathLabel} has invalid schema pattern` };
			}
		}
	}

	if (typeof value === "number") {
		if (typeof s.minimum === "number" && value < s.minimum) return { valid: false, reason: `${pathLabel} below minimum ${s.minimum}` };
		if (typeof s.maximum === "number" && value > s.maximum) return { valid: false, reason: `${pathLabel} above maximum ${s.maximum}` };
		if (typeof s.exclusiveMinimum === "number" && value <= s.exclusiveMinimum) return { valid: false, reason: `${pathLabel} below/equal exclusiveMinimum ${s.exclusiveMinimum}` };
		if (typeof s.exclusiveMaximum === "number" && value >= s.exclusiveMaximum) return { valid: false, reason: `${pathLabel} above/equal exclusiveMaximum ${s.exclusiveMaximum}` };
	}

	if (Array.isArray(value)) {
		if (typeof s.minItems === "number" && value.length < s.minItems) return { valid: false, reason: `${pathLabel} has fewer than ${s.minItems} items` };
		if (typeof s.maxItems === "number" && value.length > s.maxItems) return { valid: false, reason: `${pathLabel} has more than ${s.maxItems} items` };
		if (s.items !== undefined) {
			for (let i = 0; i < value.length; i++) {
				const result = validateJsonSchemaValue(s.items, value[i], `${pathLabel}[${i}]`, rootSchema);
				if (!result.valid) return result;
			}
		}
	}
	...
	if (isPlainJsonObject(value)) {
		const properties = isPlainJsonObject(s.properties) ? s.properties as Record<string, unknown> : {};
		if (Array.isArray(s.required)) {
			for (const key of s.required) {
				if (typeof key === "string" && !(key in value)) return { valid: false, reason: `${pathLabel}.${key} is required` };
			}
		}
		for (const [key, childSchema] of Object.entries(properties)) {
			if (key in value) {
				const result = validateJsonSchemaValue(childSchema, value[key], `${pathLabel}.${key}`, rootSchema);
				if (!result.valid) return result;
			}
		}
		for (const key of Object.keys(value)) {
			if (key in properties) continue;
			if (s.additionalProperties === false) return { valid: false, reason: `${pathLabel}.${key} is not allowed` };
			if (s.additionalProperties && typeof s.additionalProperties === "object") {
				const result = validateJsonSchemaValue(s.additionalProperties, value[key], `${pathLabel}.${key}`, rootSchema);
				if (!result.valid) return result;
			}
		}
	}
...
function matchesJsonSchemaType(typeName: string, value: any): boolean {
	if (typeName === "null") return value === null;
	if (typeName === "array") return Array.isArray(value);
	if (typeName === "object") return isPlainJsonObject(value);
...
function deepEqualJson(a: any, b: any): boolean {
	...
	if (isPlainJsonObject(a) || isPlainJsonObject(b)) {
		...
		return aKeys.every((key) => Object.prototype.hasOwnProperty.call(b, key) && deepEqualJson(a[key], b[key]));
	}
	return false;
}
...
// IDC-LOCAL — resolve this agent's IDC role for the glass-wall ACL. Prefer the explicit
// signals (env, then the --idc-role flag the role harness registers), falling back to the
// coms-net peer name (idc-pi launches each role with `--name <role>`). Returns undefined
// when no IDC role is in play, so the ACL fail-closes (non-IDC sessions cannot send to
// IDC role peers by name).
function readIdcRoleForComsNet(pi: ExtensionAPI, identityName: string | undefined): string | undefined {
	const env = process.env.PI_IDC_ROLE;
	if (env && env.trim()) return env.trim();
	try {
		const flag = pi.getFlag("idc-role") as string | undefined;
		if (flag && flag.trim()) return flag.trim();
	} catch { /* flag not registered when the role harness isn't co-loaded */ }
	return identityName && identityName.trim() ? identityName.trim() : undefined;
}
...
export default function (pi: ExtensionAPI) {
	...
	function messageRouteQuery(extra?: Record<string, string | number>): string {
	...
	function handleSseEvent(event: string, data: any, _id?: string): void {
		...
		switch (event) {
			...
			case "server_shutdown": {
				...
				audit("server_shutdown", { reason, exit_on_server_shutdown: EXIT_ON_SERVER_SHUTDOWN });
				if (!EXIT_ON_SERVER_SHUTDOWN) {
					if (currentCtx?.hasUI) {
						try { currentCtx.ui.notify("📡 coms-net: hub shut down — staying alive (opt-out)", "warning"); } catch { /* ignore */ }
					}
					return;
				}
				if (currentCtx?.hasUI) {
					try { currentCtx.ui.notify("📡 coms-net: hub shut down — exiting session", "warning"); } catch { /* ignore */ }
				}
				void (async () => {
					await cleanShutdown();
					try { currentCtx?.shutdown?.(); } catch { /* ignore */ }
				})();
				return;
			}
			case "error": {
	...
	pi.registerTool({
		...
		async execute(_callId, params) {
			...
			const acl = evaluateComsNetSendForRole(senderRole ?? "", params.target);
			if (!acl.allowed) {
				try {
					pi.appendEntry("coms-net-log", {
						...
						sender_role: acl.senderRole ?? senderRole ?? null,
						target_role: acl.targetRole ?? null,
						reason: acl.reason,
					});
				} catch { /* best-effort */ }
				throw new Error(`coms-net: glass-wall ACL blocked send → ${params.target}: ${acl.reason}`);
			}
	...
	pi.registerTool({
		...
		async execute(_callId, params) {
			...
			let resp: any;
			try {
				resp = await httpFetch("GET", `/v1/messages/${encodeURIComponent(msg_id)}?${messageRouteQuery()}`);
			} catch (err) {
	...
	pi.registerTool({
		...
		async execute(_callId, params) {
			...
			const serverPromise = httpFetch(
				"GET",
				`/v1/messages/${encodeURIComponent(msg_id)}/await?${messageRouteQuery({ timeout_ms: serverTimeoutMs })}`,
				undefined,
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		const { payload, error } = prepareComsNetResponsePayload(lastAssistantText, inbound.response_schema ?? null);
