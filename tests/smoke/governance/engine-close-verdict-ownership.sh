...
V_FOR_A="$REPO/docs/workflow/code-reviews/2026-07-22-pr-9-for-A.json"
cat > "$V_FOR_A" <<JSON
{"verdict":"PASS","pr":9,"issue":$a,"findings":[]}
JSON
python3 "$CHECK" "$V_FOR_A" >/dev/null 2>&1 || fail "valid PASS verdict for A did not validate"
...
V_NOPR="$REPO/docs/workflow/code-reviews/2026-07-22-no-pr-for-A.json"
cat > "$V_NOPR" <<JSON
{"verdict":"PASS","issue":$a,"findings":[]}
JSON
python3 "$CHECK" "$V_NOPR" >/dev/null 2>&1 || fail "valid no-pr verdict for A did not validate"

# (1) using A's verdict to close B ⇒ denied (verdict.issue != B).
if eng close --num "$b" --verdict "$V_FOR_A" --pr 9 2>/dev/null; then
...
if eng close --num "$a" --verdict "$V_NOPR" 2>/dev/null; then
...
if eng close --num "$a" --verdict "$V_FOR_A" --pr 7 2>/dev/null; then
...
eng close --num "$a" --verdict "$V_FOR_A" --pr 9 >/dev/null 2>&1 \
