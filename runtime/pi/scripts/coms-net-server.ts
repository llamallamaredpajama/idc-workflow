// ─────────────────────────────────────────────────────────────────────────────
// VENDORED from pi-harnesses · upstream path: scripts/coms-net-server.ts
...
// Bun drops newly-enqueued SSE frames if we synchronously close streams/server
// in the same tick. Keep this flush window before destructive fleet teardown.
...
function broadcastServerShutdown(reason: string): void {
	for (const [projectName, p] of state.projects) {
		broadcast(p, "server_shutdown", { project: projectName, reason });
	}
}
...
function lookupMessageForSender(req: Request, url: URL, msg_id: string): { project: ProjectState; msg: ComsMessage } | Response {
	...
	if (!senderSession) return errorJson("missing_sender_session", 400);
...
async function handleSubmitResponse(
	...
	if (
		...
		typeof body !== "object" ||
		typeof body.project !== "string" ||
		body.project.length === 0 ||
		typeof body.responder_session !== "string"
	) {
		return errorJson("invalid_request", 400);
	}
	const project = state.projects.get(body.project);
	const msg = project?.messages.get(msg_id);
	if (!project || !msg) return errorJson("message_not_found", 404);
...
export function __resetComsNetServerForTests(authToken = "test-token"): void {
	TOKEN = authToken;
	TOKEN_FILE_OWNED_BY_US = false;
	state.server_id = ulid();
	state.started_at = nowIso();
	state.projects.clear();
}

export function __dispatchComsNetRequestForTests(req: Request): Promise<Response> {
	return router(req);
}

export function __broadcastServerShutdownForTests(reason: string): void {
	broadcastServerShutdown(reason);
}
...
export function main(): void {
	...
	if (!LOG_QUIET) {
		console.log(`${bootDim}          ─── events below (Ctrl-C = fleet teardown; kill -TERM = hub only; set PI_COMS_NET_LOG_HEARTBEAT=1 for heartbeat noise) ───${bootReset}`);
	}
	...
	const shutdown = (sig: string) => {
		...
		const fleetTeardown = sig === "SIGINT";
		if (fleetTeardown) {
			broadcastServerShutdown("operator_sigint");
			// Intentionally not unref'd: delivery-critical frame must get its window.
			setTimeout(finishShutdown, SHUTDOWN_FLUSH_MS);
		} else {
			finishShutdown();
		}
	};
