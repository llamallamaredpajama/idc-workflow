...
- **Iteration policy** — record-and-vary: log each finding, the fix applied, and the re-review
  ...
  rejected/refuted/below-floor finding is a recognized *seen* resurfacing — never scored as new
  work, and never re-filed as duplicate **routed** work (suppression requires the board to
  corroborate that the work really is routed; a resurfacing the board does not carry is filed, so
  nothing silently vanishes) — and it never turns into a false routing gap at the finish tail (the
  receipt gate's routing check is seen-ledger-aware).
...
5. **Git finalization.** Acquire the area's **surface-keyed merge-train lease** (the serialized
   ...
   **Where you write the four witness artifacts is not a trap.** Build runs in a linked worktree, so
   ...
   never moves. This is the ONE exception to "an uncommitted file is destroyed at finalization" in
   step 4 — it applies to those four receipt-named artifacts only, never to source or docs.
