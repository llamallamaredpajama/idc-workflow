...
2. **Phase-close delta review** — one pass over the phase delta; findings are routed to the
   board deterministically by the verdict gate + `scripts/idc_file_findings.py` filer (non-blocking
   `Stage=Recirculation` items; phase close does not drive them to zero) — **except
