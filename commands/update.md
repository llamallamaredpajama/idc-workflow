...
walk-away from an open command). Update is a **resync/maintenance** command — no pipeline handoff, so
the next-action oracle (`scripts/idc_next_action.py`) is not called here:
