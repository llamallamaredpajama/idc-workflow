...
# (3) Stage/Status are NEVER pre-seeded: their option sets are the machine's own enums.
STAGE_BEFORE="$(opt_names Stage)"
timeout "$T_S" env PATH="$BIN:$PATH" python3 - "$PLUGIN/scripts" "$REPO" <<'PY' >"$WORK/stage.out" 2>&1
import sys
scripts, repo = sys.argv[1:3]
sys.path.insert(0, scripts)
import idc_tracker_transaction as TX
print("preseed_fields=%s" % (TX._PRESEED_FIELDS,))
PY
grep -q "preseed_fields=('Wave', 'Phase', 'Domain')" "$WORK/stage.out" \
  || fail "#208: the pre-seed field set must be exactly Wave/Phase/Domain. Status and Stage are the
machine's own enums (workflow-machine.yaml) — minting one from a plan's data would let a transaction
invent a workflow state. Got: $(cat "$WORK/stage.out")"
[ "$(opt_names Stage)" = "$STAGE_BEFORE" ] \
  || fail "#208: the Stage option set changed during pre-seeding"
...
echo "PASS: a plan beyond wave 1 pre-seeds exactly the Wave/Phase/Domain options its frozen transaction needs, non-destructively (existing node ids survive), idempotently, never touching the Status/Stage enums, and fail-soft when the board cannot be read"
