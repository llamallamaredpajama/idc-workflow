#!/usr/bin/env python3
"""idc_ripple_layers.py — Ripple's downstream sync set + gate decision (`WORKFLOW.md §4.4`).

Ripple is the only retrograde path, and it obeys one-way flow: changing a canonical layer
...
decision — the deterministic core of the ripple analysis.

Usage: idc_ripple_layers.py <prd|spec|master|subphase|pillar>   (exit 0 ok, 2 = bad layer)
"""
import sys
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in CHAIN:
        sys.stderr.write(f"usage: idc_ripple_layers.py <{'|'.join(CHAIN)}>\n")
        sys.exit(2)
    layer = sys.argv[1]
